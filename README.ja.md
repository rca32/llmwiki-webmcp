...
Project Auroraのローンチレビューを開いたとします。ページは一文だけで、決定、
ローンチ条件、それを裏づける根拠はまだ整理されていません。

エディターを開く代わりに、望む結果を依頼します。

```text
添付したローンチレビューのメモを、Project Auroraの正式な意思決定ページに
まとめてください。この既存ページを再利用し、決定、ローンチ条件、現在の根拠、
未解決の質問を分けてください。出典をリンクし、不確実性を残したまま、未達の
条件を完了済みとして書かないでください。
```

エージェントは次の作業を行います。

- 現在のページ、バージョン、許可された範囲を確認する。
- 新規作成や重複の前に、既存のWikiを検索する。
- 関連ページ、出典、つながりを調べる。
- 開いているページのWebMCPツールを通じて変更を適用する。
- 根拠と変更履歴を含む結果を同じWikiに残す。
