...
Project Aurora 출시 검토 문서를 열었다고 생각해 보세요. 내용은 한 줄뿐이고,
결정 사항과 출시 조건, 이를 뒷받침할 근거는 아직 정리되지 않았습니다.

편집기를 여는 대신 원하는 결과를 요청합니다.

```text
첨부한 출시 검토 메모를 Project Aurora의 정본 의사결정 문서로 만들어 주세요.
이 기존 문서를 재사용하고, 결정 사항과 출시 조건, 현재 근거, 열린 질문을 나눠
주세요. 출처를 연결하고 불확실성을 보존하며, 아직 충족되지 않은 조건을 완료된
것처럼 쓰지 마세요.
```

에이전트는 다음 작업을 수행합니다.

- 현재 문서와 버전, 허용된 범위를 확인합니다.
- 새로 만들거나 중복시키기 전에 기존 위키를 검색합니다.
- 관련 문서와 출처, 연결을 살펴봅니다.
- 열린 페이지의 WebMCP 도구로 변경을 적용합니다.
- 근거와 변경 기록을 포함한 결과를 같은 위키에 남깁니다.
