...
export async function GET(_request: Request, { params }: Context) {
  ...
  try {
    ...
    return Response.json(
      success({ page: await getPage(session.wikiId!, pageId) }, id),
...
export async function PATCH(request: Request, { params }: Context) {
  ...
  try {
    ...
    const { pageId } = await params,
      ...
      saveKind =
        origin === "human" && body.save_kind === "autosave"
          ? "autosave"
          : undefined;
    const result = await updatePage({
      ...
      changeSummary: requiredString(
        body.change_summary ??
          (saveKind === "autosave" ? "Autosave" : "Page updated"),
        "change_summary",
        1,
        500,
      ),
...
export async function DELETE(request: Request, { params }: Context) {
  ...
  try {
    ...
    const result = await softDeletePage({
      ...
      reason: requiredString(body.reason, "reason", 1, 500),
      operationId: operationId(body.operation_id),
      requestId: id,
      origin: originFrom(request),
    });
