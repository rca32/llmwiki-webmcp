...
| `src/components/editor/wiki-editor.tsx`    | `site/components/editor/wiki-editor.tsx`                                  | Adapted reader shell           | Replaces Milkdown, filesystem writes, and built-in editing with an always-preview Markdown reader. The retained document header and status bar host non-mutating copy actions and one contextual change-request action; no edit mode, draft, save, autosave, move, or delete callback remains.                                                                                         |
...
icon-rail, read-only page-tree, Markdown preview, and revision-panel layout was
