...
假设你打开 Project Aurora 的发布评审页面。页面只有一句话，决策、发布条件和
支持证据都还没有整理进去。

你不必打开编辑器，只需描述想要的结果：

```text
请把附上的发布评审笔记整理成 Project Aurora 的规范决策页面。复用当前页面，
分别列出决策、发布条件、现有证据和待解决问题。链接来源、保留不确定性，不要把
尚未满足的条件写成已经完成。
```
...
- 在新建或重复内容之前搜索现有 Wiki；
- 查看相关页面、来源和关联；
- 通过当前页面提供的 WebMCP 工具应用更改；
- 把带有证据和修改历史的结果留在同一个 Wiki 中。
