...
import { extractWikiLinks,operationId,parseFrontmatter,slugify,stableJson } from './validation';
...
describe('wiki validation',()=>{
  ...
  it('parses bounded scalar and string-array frontmatter',()=>{ expect(parseFrontmatter('---\ntags: ["webmcp", "wiki"]\ndraft: false\nweight: 3\n---\n# Page')).toEqual({tags:['webmcp','wiki'],draft:false,weight:3}); });
  it('rejects malformed frontmatter instead of silently dropping it',()=>{ expect(()=>parseFrontmatter('---\ninvalid line\n---\n# Page')).toThrow(/frontmatter/i); });
});
